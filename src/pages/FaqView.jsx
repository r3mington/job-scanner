      ...
      details: (
        <div className="space-y-3 leading-relaxed text-slate-350">
          ...
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li><strong>Automated Fallback Cascades:</strong> To protect batch operations from API downtime or individual key rate limits, the system runs an automatic fallback chain. If a request fails, the service retries transient rate-limits on the same model, then cycles through backup models (<code>gemini-2.5-flash</code> → <code>gemini-2.5-flash-lite</code> → <code>gemini-2.0-flash</code> → <code>gemini-2.5-pro</code>) in real-time.</li>
          </ul>
          <p>
